...
class JanusT2IR1Trainer(Trainer):
    ...
    def __init__(
        ...
        # script_args
        self.self_uncertainty = script_args.self_uncertainty
        self.self_uncertainty_beta = script_args.self_uncertainty_beta
        self.reward_weight = script_args.reward_weight
        self.self_uncertainty_part = script_args.self_uncertainty_part
        ...
        if not self.use_image_cot:
            ...
            assert self.self_uncertainty_part == 'image'
    ...
    def _get_per_token_self_uncertainty(self, model, input_embeds, text_ids, img_ids, attention_mask):
        def _get_forward_KL(logits, input_ids):
            ...
            for logits_row, input_ids_row in zip(logits, input_ids):
                token_self_uncertainty = torch.logsumexp(logits_row, dim=-1) - logits_row.mean(dim=-1)
        ...
        if img_ids is not None:
            ...
            img_input_ids = torch.cat([img_ids.new_zeros(img_ids.size(0), 1), img_ids], dim=1) # cat a random one here, since it is not used in the loss calculation
            per_token_iris_img = NSC(image_logits, img_input_ids) # only calculate image loss
            return torch.cat([
                per_token_iris_img.new_zeros(
                    (per_token_iris_img.size(0), input_embeds.size(1) - per_token_iris_img.size(1) - 1)
                ), # the return length should be the input length minus 1 (the last token does not need predict)
                per_token_iris_img
            ], 
            dim=1)
            ...
            last_hidden_states = hidden_states[-1]
            text_logits = model.language_model.lm_head(last_hidden_states) 
            per_token_iris_text = NSC(text_logits, text_ids) 
            return per_token_iris_text
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if self.use_image_cot:
            ...
            raise NotImplementedError("Currently only image cot is supported.")
    ...
    def compute_loss_with_image_cot(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        per_token_self_uncertainty = self._get_per_token_self_uncertainty(
            model=model.module,
            input_embeds=input_embeds,
            text_ids=prompt_all_ids,
            img_ids=None,
        ...
        per_token_self_uncertainty = per_token_self_uncertainty[:, prompt_length - 1 :]
        text_certainty_mean = per_token_self_uncertainty.mean(dim=-1, keepdim=True)
        ...
        loss_dict['semantic-cot'] = {
            ...
            'per_token_self_uncertainty': per_token_self_uncertainty,
        }
        torch.set_grad_enabled(False)
        ...
        per_token_logps_semantic, ref_per_token_logps_semantic, completion_mask_semantic, per_token_self_uncertainty_semantic = [], [], [], []
        per_token_logps_token, ref_per_token_logps_token, completion_mask_token, per_token_self_uncertainty_token = [], [], [], []
        
        for key in ['semantic-cot', 'token-cot']:
            if loss_dict[key]['per_token_logps'] is None:
                loss_dict[key]['loss'] = None
                continue
            if key == 'semantic-cot' and (self.self_uncertainty_part == 'all' or self.self_uncertainty_part == 'text'):
                ...
                per_token_self_uncertainty_semantic.append(loss_dict[key]['per_token_self_uncertainty'])
                ...
                per_token_self_uncertainty_semantic = torch.cat(per_token_self_uncertainty_semantic, dim=1)

            elif key == 'token-cot' and (self.self_uncertainty_part == 'all' or self.self_uncertainty_part == 'image'):
                ...
                per_token_self_uncertainty_token.append(loss_dict[key]['per_token_self_uncertainty'])
                ...
                per_token_self_uncertainty_token = torch.cat(per_token_self_uncertainty_token, dim=1)
        ...
        per_token_self_uncertainty_all = torch.cat([per_token_self_uncertainty_semantic, per_token_self_uncertainty_token], dim=1)

        def compute_advantage(completion_mask, per_token_self_uncertainty):

            sce_advantage_raw = self._get_advantage_from_sce(
                SCe=per_token_self_uncertainty,
                completion_mask=completion_mask,
            )
        ...
        if self.self_uncertainty:
            sce_advantage_raw_semantic = self._get_advantage_from_sce(SCe=per_token_self_uncertainty_semantic, completion_mask=completion_mask_semantic)
            sce_advantage_raw_token = self._get_advantage_from_sce(SCe=per_token_self_uncertainty_token, completion_mask=completion_mask_token)
            sce_advantage_all = compute_advantage(completion_mask_all, per_token_self_uncertainty_all)
            
            
            advantage_all = sce_advantage_all * self.self_uncertainty_beta + advantages * self.reward_weight
        ...
        self._metrics["self_uncertainty_semantic"].append(self.accelerator.gather_for_metrics(sce_advantage_raw_semantic).mean().item())
        self._metrics["self_uncertainty_token"].append(self.accelerator.gather_for_metrics(sce_advantage_raw_token).mean().item())
