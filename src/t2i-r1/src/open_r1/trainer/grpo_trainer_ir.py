...
class JanusT2IR1Trainer(Trainer):
    ...
    def __init__(
        ...
        # script_args
        self.self_certainty = script_args.self_certainty
        self.self_certainty_beta = script_args.self_certainty_beta
        self.reward_weight = script_args.reward_weight
        self.self_certainty_part = script_args.self_certainty_part
        self.logprob_part = script_args.logprob_part
        self.use_image_cot = script_args.use_image_cot
        self.self_certainty_alpha = script_args.self_certainty_alpha
        ...
        for i, reward_func in enumerate(reward_funcs):
            ...
            elif isinstance(reward_func, str) and 'entropy' in reward_func:
                pass
            else:
        ...
        if not self.use_image_cot:
            assert self.num_generations == 1
            assert self.self_certainty_part == 'image'
    ...
    def _get_per_token_self_certainty(self, model, input_embeds, text_ids, img_ids, attention_mask):
        def _get_forward_KL(logits, input_ids):
            logits = logits[:, :-1, :]  # (B, L-1, V), exclude the last logit: it corresponds to the next token pred
            input_ids = input_ids[:, 1:]  # (B, L-1), exclude the first input ID since we don't have logits for it
            # Compute the log probabilities for the input tokens. Use a loop to reduce memory peak.
            per_token_self_certainty = []

            for logits_row, input_ids_row in zip(logits, input_ids):
                token_self_certainty = torch.logsumexp(logits_row, dim=-1) - logits_row.mean(dim=-1)
                per_token_self_certainty.append(token_self_certainty)
            return -torch.stack(per_token_self_certainty)
        # here, we only compute either text or image loss, so ids of other one could be omitted
        def _get_backward_KL(logits, input_ids):
            logits = logits[:, :-1, :]  # (B, L-1, V), exclude the last logit: it corresponds to the next token pred
            input_ids = input_ids[:, 1:]  # (B, L-1), exclude the first input ID since we don't have logits for it
            # Compute the log probabilities for the input tokens. Use a loop to reduce memory peak.
            per_token_self_uncertainty = []

            for logits_row, input_ids_row in zip(logits, input_ids):
                log_probs = logits_row.log_softmax(dim=-1)
                token_self_uncertainty = -torch.sum(log_probs * log_probs.exp(), dim=-1)  # entropy
                per_token_self_uncertainty.append(token_self_uncertainty)
            return torch.stack(per_token_self_uncertainty)
        
        IRIS = None
        if self.script_args.iris_type == "forward_kl":
            IRIS = _get_forward_KL
        elif self.script_args.iris_type == "backward_kl":
            IRIS = _get_backward_KL
        else:
        ...
        if img_ids is not None:
            ...
            img_input_ids = torch.cat([img_ids.new_zeros(img_ids.size(0), 1), img_ids], dim=1) # cat a random one here, since it is not used in the loss calculation
            per_token_logps_img = IRIS(image_logits, img_input_ids) # only calculate image loss
            return torch.cat([
                per_token_logps_img.new_zeros(
                    (per_token_logps_img.size(0), input_embeds.size(1) - per_token_logps_img.size(1) - 1)
                ), # the return length should be the input length minus 1 (the last token does not need predict)
            ...
            last_hidden_states = hidden_states[-1]
            text_logits = model.language_model.lm_head(last_hidden_states) 
            per_token_logps_text = IRIS(text_logits, text_ids) 
            return per_token_logps_text
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if self.use_image_cot:
            return self.compute_loss_with_image_cot(model, inputs, return_outputs, num_items_in_batch)
        else:
            return self.compute_loss_without_image_cot(model, inputs, return_outputs, num_items_in_batch)        
        
    def compute_loss_without_image_cot(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if return_outputs:
        ...
        for image_gen_prompt in prompts:
            conversation = [
                {
                    "role": "<|User|>",
        ...
        per_token_self_certainty = self._get_per_token_self_certainty(
            model=model.module,
            input_embeds=input_embeds,
            text_ids=None,
            img_ids=total_generated_tokens_img,
        ...
        per_token_self_certainty = per_token_self_certainty[:, prompt_length - 1 :]
        ...
        loss_dict['token-cot'] = {
            ...
            'per_token_self_certainty': per_token_self_certainty,
        }
        torch.set_grad_enabled(False)
        ...
        per_token_logps, ref_per_token_logps, completion_mask, per_token_self_certainty = [], [], [], []
        
        
        per_token_logps.append(loss_dict['token-cot']['per_token_logps'])
        ref_per_token_logps.append(loss_dict['token-cot']['ref_per_token_logps'])
        completion_mask.append(loss_dict['token-cot']['completion_mask'])
        per_token_self_certainty.append(loss_dict['token-cot']['per_token_self_certainty'])
        ...
        per_token_self_certainty = torch.cat(per_token_self_certainty, dim=1)
        
        # Compute the self-certainty advantage
        sce_advantage_raw = self._get_advantage_from_sce(
            SCe=per_token_self_certainty,
            completion_mask=completion_mask,
        )
        
        sce_advantage_mean = sce_advantage_raw.view(-1, self.num_generations * self.new_generations_image).mean(dim=1).repeat_interleave(self.num_generations * self.new_generations_image)
        sce_advantage_std = sce_advantage_raw.view(-1, self.num_generations * self.new_generations_image).std(dim=1).repeat_interleave(self.num_generations * self.new_generations_image)
        sce_advantage = (sce_advantage_raw - sce_advantage_mean)
        sce_advantage = sce_advantage / (sce_advantage_std + 1e-4)
        
        if self.self_certainty:
            advantages = sce_advantage * self.self_certainty_beta + advantages * self.reward_weight
        ...
        self._metrics["self_certainty"].append(self.accelerator.gather_for_metrics(sce_advantage_raw).mean().item())

        return loss
    
    def compute_loss_with_image_cot(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if return_outputs:
            raise ValueError("The GRPOTrainer does not support returning outputs")
        ...
        #     layer.self_attn.forward = MethodType(functools.partial(flex_attention_forward, prompt_length=prompt_length), layer.self_attn)

        # per_token_logps_sliding = self._get_per_token_logps(
        #     model=model.module, 
        #     input_embeds=input_embeds,
        #     text_ids=prompt_all_ids, 
        #     img_ids=None, 
        ...
        per_token_logps = per_token_logps[:, prompt_length - 1 :]
        per_token_self_certainty = per_token_self_certainty[:, prompt_length - 1 :]
        text_certainty_mean = per_token_self_certainty.mean(dim=-1, keepdim=True)

        # thresholds = torch.topk(per_token_self_certainty, per_token_self_certainty.shape[1], dim=1).values[:, -1].unsqueeze(1)
        # top_mask = per_token_self_certainty >= thresholds
        ...
        image_certainty_mean = per_token_self_certainty.mean(dim=-1, keepdim=True)
        # per_token_self_certainty *= text_certainty_mean / (image_certainty_mean+1e-8)
        ...
        per_token_logps_semantic, ref_per_token_logps_semantic, completion_mask_semantic, per_token_self_certainty_semantic = [], [], [], []
        per_token_logps_token, ref_per_token_logps_token, completion_mask_token, per_token_self_certainty_token = [], [], [], []
        
        for key in ['semantic-cot', 'token-cot']:
            if loss_dict[key]['per_token_logps'] is None:
                loss_dict[key]['loss'] = None
                continue
            if key == 'semantic-cot' and (self.self_certainty_part == 'all' or self.self_certainty_part == 'text'):
                per_token_logps_semantic.append(loss_dict[key]['per_token_logps'])
                ref_per_token_logps_semantic.append(loss_dict[key]['ref_per_token_logps'])
                completion_mask_semantic.append(loss_dict[key]['completion_mask'])
                per_token_self_certainty_semantic.append(loss_dict[key]['per_token_self_certainty'])
                per_token_logps_semantic = torch.cat(per_token_logps_semantic, dim=1)
                ref_per_token_logps_semantic = torch.cat(ref_per_token_logps_semantic, dim=1)
                completion_mask_semantic = torch.cat(completion_mask_semantic, dim=1)
                per_token_self_certainty_semantic = torch.cat(per_token_self_certainty_semantic, dim=1)

            elif key == 'token-cot' and (self.self_certainty_part == 'all' or self.self_certainty_part == 'image'):
                per_token_logps_token.append(loss_dict[key]['per_token_logps'])
                ref_per_token_logps_token.append(loss_dict[key]['ref_per_token_logps'])
                completion_mask_token.append(loss_dict[key]['completion_mask'])
                per_token_self_certainty_token.append(loss_dict[key]['per_token_self_certainty'])
                per_token_logps_token = torch.cat(per_token_logps_token, dim=1)
                ref_per_token_logps_token = torch.cat(ref_per_token_logps_token, dim=1)
                completion_mask_token = torch.cat(completion_mask_token, dim=1)
                per_token_self_certainty_token = torch.cat(per_token_self_certainty_token, dim=1)

        per_token_logps_all = torch.cat([per_token_logps_semantic, per_token_logps_token], dim=1)
        ref_per_token_logps_all = torch.cat([ref_per_token_logps_semantic, ref_per_token_logps_token], dim=1)
        completion_mask_all = torch.cat([completion_mask_semantic, completion_mask_token], dim=1)
        per_token_self_certainty_all = torch.cat([per_token_self_certainty_semantic, per_token_self_certainty_token], dim=1)

        def compute_advantage(completion_mask, per_token_self_certainty):
            ...
            return sce_advantage
        
        def compute_loss_kl(per_token_logps, ref_per_token_logps, completion_mask, advantage):
            per_token_loss = torch.exp(per_token_logps - per_token_logps.detach()) * advantage.unsqueeze(1)
            per_token_kl = torch.exp(ref_per_token_logps - per_token_logps) - (ref_per_token_logps - per_token_logps) - 1
            per_token_loss = -(per_token_loss - self.beta * per_token_kl)
            # loss = (per_token_loss * completion_mask).sum() / completion_mask.sum()
            # mean_kl = (per_token_kl * completion_mask).sum() / completion_mask.sum()
        ...
        if self.self_certainty:
            sce_advantage_raw_semantic = self._get_advantage_from_sce(SCe=per_token_self_certainty_semantic, completion_mask=completion_mask_semantic)
            sce_advantage_raw_token = self._get_advantage_from_sce(SCe=per_token_self_certainty_token, completion_mask=completion_mask_token)
            sce_advantage_semantic = compute_advantage(completion_mask_semantic, per_token_self_certainty_semantic)
            sce_advantage_token = compute_advantage(completion_mask_token, per_token_self_certainty_token)
            sce_advantage_all = compute_advantage(completion_mask_all, per_token_self_certainty_all)
            
            
            if self.script_args.loss_text_img_separate: # two advantage
                advantage_semantic = sce_advantage_semantic * self.self_certainty_beta + advantages * self.reward_weight
                advantage_token = sce_advantage_token * self.self_certainty_beta + advantages * self.reward_weight
            else: # only one advantage
                if self.script_args.adv_text_img_separate:
                    advantage_all = (sce_advantage_semantic + sce_advantage_token) / 2 * self.self_certainty_beta + advantages * self.reward_weight
                else:
                    advantage_all = sce_advantage_all * self.self_certainty_beta + advantages * self.reward_weight
                advantage_semantic = advantage_all
                advantage_token = advantage_all
                
            
            
            if self.script_args.mask_text_img_separate:
                per_token_loss_semantic, per_token_kl_semantic = compute_loss_kl(per_token_logps_semantic, ref_per_token_logps_semantic, completion_mask_semantic, advantage_semantic)
                per_token_loss_token, per_token_kl_token = compute_loss_kl(per_token_logps_token, ref_per_token_logps_token, completion_mask_token, advantage_token)
                loss_semantic = (per_token_loss_semantic * completion_mask_semantic).sum() / completion_mask_semantic.sum()
                loss_token = (per_token_loss_token * completion_mask_token).sum() / completion_mask_token.sum()
                mean_kl_semantic = (per_token_kl_semantic * completion_mask_semantic).sum() / completion_mask_semantic.sum()
                mean_kl_token = (per_token_kl_token * completion_mask_token).sum() / completion_mask_token.sum()
                if self.self_certainty_alpha > 0:
                    mean_kl = mean_kl_semantic * self.self_certainty_alpha + mean_kl_token
                    loss = loss_semantic * self.self_certainty_alpha + loss_token
                elif self.self_certainty_alpha == -1:
                    mean_kl = mean_kl_semantic
                    loss = loss_semantic
            else:
                per_token_loss_all, per_token_kl_all = compute_loss_kl(per_token_logps_all, ref_per_token_logps_all, completion_mask_all, advantage_all)
                loss = (per_token_loss_all * completion_mask_all).sum() / completion_mask_all.sum()
                mean_kl = (per_token_kl_all * completion_mask_all).sum() / completion_mask_all.sum()
        ...
        self._metrics["self_certainty_semantic"].append(self.accelerator.gather_for_metrics(sce_advantage_raw_semantic).mean().item())
        self._metrics["self_certainty_token"].append(self.accelerator.gather_for_metrics(sce_advantage_raw_token).mean().item())
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        logs = {**logs, **metrics}
        if self.is_world_process_zero() and is_wandb_available() and wandb.run is not None:
            wandb.log(logs)
